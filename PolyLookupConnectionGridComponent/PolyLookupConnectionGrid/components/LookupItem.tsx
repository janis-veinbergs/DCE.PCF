...
/* Allows picking which entity to filter on */
export type ILookupItemEntity = ITag & {
  logicalName: string;
  entityIconUrl: string | null;
};
export type ILookupPossibleItems = ILookupItem | ILookupItemEntity;
export const isLookupItemEntity = (item: ILookupPossibleItems): item is ILookupItemEntity => "logicalName" in item;
export const isLookupItem = (item: ILookupPossibleItems): item is ILookupItem => "data" in item;
export type ILookupItemProps = IPickerItemProps<ILookupPossibleItems> &
  ...
  ({
    ...
    return (
      ...
      >
        ...
        {isLookupItemEntity(item) && <div>{item.name}</div>}
      </TagItem>
    );
  },
  (prevProps, nextProps) => "key" in prevProps.item && "key" in nextProps.item && prevProps.item.key === nextProps.item.key
);
